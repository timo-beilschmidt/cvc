...
#pragma omp parallel
{
  ...
  double ** propagator_up_src_smeared = init_2level_dtable ( 12, _GSI( VOLUME ) );
  if( propagator_up_src_smeared == NULL ) {
    fprintf(stderr, "[njn_fht_invert_contract] Error from init_2level_dtable %s %d\n", __FILE__, __LINE__);
    EXIT(123);
  }
  ...
  double ** propagator_dn_src_smeared = init_2level_dtable ( 12, _GSI( VOLUME ) );
  if( propagator_dn_src_smeared == NULL ) {
    fprintf(stderr, "[njn_fht_invert_contract] Error from init_2level_dtable %s %d\n", __FILE__, __LINE__);
    EXIT(123);
  }
...
#else
#  error "need AFF library; no other I/O method than AFF available"
#endif
     ...
     ***********************************************************/
    exitstatus = point_source_propagator ( propagator_up, gsx, _OP_ID_UP, 1, 1, NULL, check_propagator_residual, gauge_field_with_phase, lmzz );
    if(exitstatus != 0) {
    ...
    /***********************************************************
     * up-type point-to-all propagator only source smeared
     ***********************************************************/
    exitstatus = point_source_propagator ( propagator_up_src_smeared, gsx, _OP_ID_UP, 1, 0, NULL, check_propagator_residual, gauge_field_with_phase, lmzz );
    if(exitstatus != 0) {
      fprintf(stderr, "[njn_fht_invert_contract] Error from point_source_propagator, status was %d\n", exitstatus);
      EXIT(12);
    }
     ...
     ***********************************************************/
    exitstatus = point_source_propagator ( propagator_dn, gsx, _OP_ID_DN, 1, 1, NULL, check_propagator_residual, gauge_field_with_phase, lmzz );
    if(exitstatus != 0) {
      fprintf(stderr, "[njn_fht_invert_contract] Error from point_source_propagator, status was %d\n", exitstatus);
      EXIT(12);
    }
    ...
    /***********************************************************
     * dn-type point-to-all propagator only source smeared
     ***********************************************************/
    exitstatus = point_source_propagator ( propagator_dn_src_smeared, gsx, _OP_ID_DN, 1, 0, NULL, check_propagator_residual, gauge_field_with_phase, lmzz );
    if(exitstatus != 0) {
      fprintf(stderr, "[njn_fht_invert_contract] Error from point_source_propagator, status was %d\n", exitstatus);
      EXIT(12);
    }
    ...
    for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) {
       ...
       ***************************************************************************/
      sprintf(aff_tag, "/N-N/T%d_X%d_Y%d_Z%d/Gi_%s/Gf_%s/n1",
       ...
       * diagram n2
       *
       * same here as was done for n1
       ***************************************************************************/
      sprintf(aff_tag, "/N-N/T%d_X%d_Y%d_Z%d/Gi_%s/Gf_%s/n2",
          gsx[0], gsx[1], gsx[2], gsx[3],
          gamma_id_to_Cg_ascii[ gamma_f1_list[if1] ], gamma_id_to_Cg_ascii[ gamma_f1_list[if2] ]);
     ...
     ***************************************************************************/
    int sc1 = 12, sc2 = 12;
    double **loop_mat = new double*[sc1];
    read_matrix(loop_mat, "loop_matrix.txt", sc1, sc2);
    
    for ( int imom = 0; imom < g_seq_source_momentum_number; imom++ ) {

      double *** sequential_source = init_3level_dtable ( 3, 12,  _GSI(VOLUME) );
      ...
      for ( int isc = 0; isc < 12; isc++ ) {
        for ( int it = 0; it < T; it++ ) {
          ...
          spinor_field_eq_spinor_field_ti_complex_field ( sequential_source[0][isc] + offset, propagator_up_src_smeared[isc] + offset, (double*)(ephase[imom]), VOL3 );
        }
      }
      ...
      for ( int igamma = 0; igamma < g_sequential_source_gamma_id_number; igamma++ ) {
        ...
        for ( int isc = 0; isc < 12; isc++ ) {
          ...
          /* loop term*/
          spinor_field_eq_loop_ti_spinor_field ( sequential_source[2][isc], loop_mat, sequential_source[1][isc], VOLUME );

          spinor_field_eq_gamma_ti_spinor_field ( sequential_source[2][isc], gamma_id, sequential_source[2][isc], VOLUME );
         ...
         ***************************************************************************/
        exitstatus = prepare_propagator_from_source ( sequential_propagator, sequential_source[2], 12, _OP_ID_UP, 0, 1, NULL,
        ...
        for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) {
           ...
           * diagram t1
           ***************************************************************************/
          sprintf(aff_tag, "/N-ubGu-N/T%d_X%d_Y%d_Z%d/Gf_%s/Gc_%s/Gi_%s/QX%d_QY%d_QZ%d/t1",
              gsx[0], gsx[1], gsx[2], gsx[3],
           ...
           * diagram t3
           ***************************************************************************/
          sprintf(aff_tag, "/N-ubGu-N/T%d_X%d_Y%d_Z%d/Gf_%s/Gc_%s/Gi_%s/QX%d_QY%d_QZ%d/t3",
              gsx[0], gsx[1], gsx[2], gsx[3],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if2] ],
              gamma_id_to_ascii[gamma_id],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if1] ], 
           ...
           * diagram t4
           ***************************************************************************/
          sprintf(aff_tag, "/N-ubGu-N/T%d_X%d_Y%d_Z%d/Gf_%s/Gc_%s/Gi_%s/QX%d_QY%d_QZ%d/t4",
              gsx[0], gsx[1], gsx[2], gsx[3],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if2] ],
              gamma_id_to_ascii[gamma_id],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if1] ], 
      ...
      for ( int isc = 0; isc < 12; isc++ ) {
        for ( int it = 0; it < T; it++ ) {
          /* memory offset for timeslice it */
          size_t const offset =  it * _GSI(VOL3);
          spinor_field_eq_spinor_field_ti_complex_field ( sequential_source[0][isc] + offset, propagator_dn_src_smeared[isc] + offset, (double*)(ephase[imom]), VOL3 );
        }
      }
      ...
      for ( int igamma = 0; igamma < g_sequential_source_gamma_id_number; igamma++ ) {
         ...
         * invert the Dirac operator on the sequential source
         ***************************************************************************/
        exitstatus = prepare_propagator_from_source ( sequential_propagator, sequential_source[2], 12, _OP_ID_DN, 0, 1, NULL, check_propagator_residual, gauge_field_with_phase, lmzz, NULL );
        if ( exitstatus != 0 ) {
          fprintf ( stderr, "[njn_fht_invert_contract] Error from prepare_propagator_from_source, status was %d %s %d\n", exitstatus, __FILE__, __LINE__ );
          EXIT(123);
        }
        ...
        /***************************************************************************
         * fill the fermion propagator fp2 point_source_propagator with sequential_propagator
         ***************************************************************************/
        assign_fermion_propagator_from_spinor_field ( fp2, sequential_propagator, VOLUME);
        ...
        for ( int if2 = 0; if2 < gamma_f1_number; if2++ ) {
           ...
           * diagram t1
           ***************************************************************************/
          sprintf(aff_tag, "/N-dbGd-N/T%d_X%d_Y%d_Z%d/Gf_%s/Gc_%s/Gi_%s/QX%d_QY%d_QZ%d/t1",
              gsx[0], gsx[1], gsx[2], gsx[3],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if2] ],
              gamma_id_to_ascii[gamma_id],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if1] ], 
           ...
           * diagram t2
           ***************************************************************************/
          sprintf(aff_tag, "/N-dbGd-N/T%d_X%d_Y%d_Z%d/Gf_%s/Gc_%s/Gi_%s/QX%d_QY%d_QZ%d/t2",
              gsx[0], gsx[1], gsx[2], gsx[3],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if2] ],
              gamma_id_to_ascii[gamma_id],
              gamma_id_to_Cg_ascii[ gamma_f1_list[if1] ], 
  ...
  fini_2level_dtable ( &propagator_up_src_smeared );
  fini_2level_dtable ( &propagator_dn_src_smeared );
